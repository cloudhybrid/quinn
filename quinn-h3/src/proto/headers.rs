...
impl Field {
        ...
        V: AsRef<[u8]>,
    {
        ...
        if name.len() < 1 {
            return Err(Error::InvalidHeaderName(format!("{:?} is invalid", name)));
        }
        ...
        if name[0] == b':' {
            ...
            return Ok(match pseudo {
                ...
                PseudoType::SCHEME => Field::Scheme(to_string(value.as_ref().into())),
                PseudoType::AUTHORITY => Field::Authority(to_string(value.as_ref().into())),
                PseudoType::PATH => Field::Path(to_string(value.as_ref().into())),
                PseudoType::STATUS => {
                    Field::Status(StatusCode::from_bytes(value.as_ref().into()).or(Err(
                        Error::InvalidHeaderValue(format!(
                            "invalid status code: {}",
                            std::string::String::from_utf8_lossy(value.as_ref())
                        )),
                    ))?)
                }
            });
